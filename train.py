...
def get_args():
    ...
    parser.add_argument('--batchsize', type=int, default=8, help='Number of batch size')
    ...
    parser.add_argument('--if_apex', default=True, help='Whether to enable Apex mixed precision training')
...
def train_net(net_type, net, device, epochs, batch_size, warm_epochs, lr, weight_decay, img_shape, apex, ref, w):
    ...
    for epoch in range(epochs):
        ...
        if ref_idx > best_ref_idx:
            ...
            logging.info(f'\tEpoch {epoch + 1} saved ! Acc result(val) = {ref_idx}')
